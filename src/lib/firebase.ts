...
function peekCatalog(): AppItem[] {
  return (
    peekCache<AppItem[]>(CATALOG_KEY) ||
    peekCache<AppItem[]>('top:50') ||
    peekCache<AppItem[]>('top:24') ||
    []
  );
}

export function appsFromCatalog(opts?: {
  category?: string;
  max?: number;
  excludeId?: string;
}): AppItem[] {
...
export async function fetchAppById(id: string): Promise<AppItem | null> {
  ...
  if (warm) {
    return cachedFetch(`app:${id}`, async () => warm, TTL.app);
  }
          ...
          .map((d) => {
            ...
            return {
              ...
              message: sanitizeText(
                (data.message as string) || (data.body as string) || (data.desc as string) || (data.content as string) || '',
                300
              ),
...
async function fetchAppsPageUncached(
  ...
  const {
    db, getDocs, collection, query, where, orderBy, fbLimit, startAfter, documentId,
  } = await fs();
  ...
  let snap;
  try {
    snap = await docsPreferCache(build(true));
  } catch {
    snap = await docsPreferCache(build(false));
  }
  ...
  return cachedFetch(
    ...
    async () => {
      ...
      try {
        ...
        return fromCatalog.slice(0, max);
      }
    },
    TTL.category
  );
...
export async function fetchNewestApps(max = 20): Promise<AppItem[]> {
  return cachedFetch(
    ...
    async () => {
      const catalog = peekCatalog();
      if (catalog.length) {
        return catalog.slice().sort((a, b) => (a.id < b.id ? 1 : -1)).slice(0, max);
      }
    ...
    async () => {
      ...
      const mapDoc = (d: { id: string; data: () => Record<string, unknown> }): AppRequest => {
        ...
        if (typeof tsRaw === 'number') timestamp = tsRaw;
          ...
          try {
            timestamp = Number((tsRaw as { toMillis: () => number }).toMillis()) || 0;
          } catch {
            timestamp = 0;
          }
        } else {
      ...
      try {
        ...
        try {
          ...
          return snap.docs
            .map((d) => mapDoc(d as { id: string; data: () => Record<string, unknown> }))
            .sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
        } catch (e) {
