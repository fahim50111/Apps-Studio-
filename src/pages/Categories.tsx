  ...
  return (
    <button
      ...
      className={`flex shrink-0 items-center gap-1.5 rounded-full px-4 py-2 text-xs font-bold transition ${
        activeState ? 'bg-accent text-ink' : 'border border-line bg-panel text-mute hover:text-fg'
      }`}
    >
      {Icon && <Icon className="h-3.5 w-3.5" style={!activeState && color ? { color } : undefined} />}
      {label}
    </button>
  );
}
